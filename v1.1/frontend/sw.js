...
self.addEventListener("push", (event) => {
  ...
  const options = {
    ...
    // Unique tag per push so consecutive task-done alerts never replace each other.
    tag: data.tag || `task-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    data: { url: data.url || "./" },
    renotify: true,
  };
